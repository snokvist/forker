...
The repository is intentionally small: `forker.c` in the root contains the entire supervisor, including config parsing, argument synthesis, and child management. The expected build artifact is `wfb_supervisor`, placed alongside the source for simplicity. `wfb.conf` demonstrates the canonical layout of `[general]` radio defaults followed by multiple `[instance name]` blocks that describe aggregators, forwarders, locals, and tunnel helpers. Keep sample configs in the root so contributors can diff them easily, and document any required external binaries (e.g., `wfb_ng`, `sse_tail`) inline in the config comments.

## Build, Test, and Development Commands
```
gcc -O2 -std=c11 -Wall -Wextra -o wfb_supervisor forker.c   # build locally
